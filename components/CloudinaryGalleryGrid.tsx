...
interface CloudinaryGalleryItem {
  id: string
  public_id: string // Cloudinary public ID (path)
  alt: string
  title: string
  type: 'image' | 'video'
}

interface CloudinaryGalleryGridProps {
  items: CloudinaryGalleryItem[]
  title: string
}

export default function CloudinaryGalleryGrid({ items, title }: CloudinaryGalleryGridProps) {
  const [selectedItem, setSelectedItem] = useState<CloudinaryGalleryItem | null>(null)
